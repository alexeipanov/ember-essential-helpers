...
module('Integration | Helper | money-format', function (hooks) {
  ...
  test('various money values', async function (assert) {
    ...
    await render(hbs`{{money-format this.value "en-US" (hash currency="USD")}}`);
    assert.dom(this.element).hasText('$3.50');

    await render(hbs`{{money-format this.value "en-US" (hash currency="EUR")}}`);
    assert.dom(this.element).hasText('€3.50');

    await render(hbs`{{money-format this.value "de-DE" (hash currency="EUR")}}`);
